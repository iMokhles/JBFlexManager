...
- (id)initWithView:(UIView *)view isInScrollView:(BOOL)inScrollView {
    ...
    if (self) {
        ...
        UIViewController *controller = [IMLEXUtility viewControllerForView:view];
        if (controller) {
            _important = YES;
...
- (NSArray<FHSView *> *)children {
    ...
    return [self.view.subviews IMLEX_mapped:^id(UIView *subview, NSUInteger idx) {
...
+ (UIImage *)_snapshotView:(UIView *)view {
    ...
    [view.subviews IMLEX_forEach:^(UIView *v, NSUInteger idx) {
        if (!v.isHidden) {
            v.hidden = YES;
