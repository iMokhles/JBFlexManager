//
//  IMLEXHierarchyTableViewController.m
//  Flipboard
//
...
#import "IMLEXHierarchyTableViewCell.h"
#import "IMLEXObjectExplorerViewController.h"
#import "IMLEXObjectExplorerFactory.h"
#import "IMLEXResources.h"
#import "IMLEXWindow.h"

typedef NS_ENUM(NSUInteger, IMLEXHierarchyScope) {
    IMLEXHierarchyScopeFullHierarchy,
    IMLEXHierarchyScopeViewsAtTap
};

@interface IMLEXHierarchyTableViewController ()
...
@implementation IMLEXHierarchyTableViewController
...
- (void)viewDidLoad {
    ...
    if (self.showScopeBar) {
        ...
        self.selectedScope = IMLEXHierarchyScopeViewsAtTap;
    }
...
+ (NSArray<UIView *> *)allViewsInHierarchy:(NSArray<UIWindow *> *)windows {
    return [windows IMLEX_flatmapped:^id(UIWindow *window, NSUInteger idx) {
        if (![window isKindOfClass:[IMLEXWindow class]]) {
...
- (void)updateDisplayedViews {
    ...
    if (self.showScopeBar) {
        if (self.selectedScope == IMLEXHierarchyScopeViewsAtTap) {
            candidateViews = self.viewsAtTap;
        } else if (self.selectedScope == IMLEXHierarchyScopeFullHierarchy) {
    ...
    if (self.searchText.length) {
        self.displayedViews = [candidateViews filteredArrayUsingPredicate:[NSPredicate predicateWithBlock:^BOOL(UIView *candidateView, NSDictionary<NSString *, id> *bindings) {
            NSString *title = [IMLEXUtility descriptionForView:candidateView includingFrame:NO];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    IMLEXHierarchyTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:CellIdentifier];
    if (!cell) {
        cell = [[IMLEXHierarchyTableViewCell alloc] initWithReuseIdentifier:CellIdentifier];
    }
    
    UIView *view = self.displayedViews[indexPath.row];

    cell.textLabel.text = [IMLEXUtility descriptionForView:view includingFrame:NO];
    cell.detailTextLabel.text = [IMLEXUtility detailDescriptionForView:view];
    cell.randomColorTag = [IMLEXUtility consistentRandomColorForObject:view];
    cell.viewDepth = self.depthsForViews[view].integerValue;
    cell.indicatedViewColor = view.backgroundColor;

    if (view.isHidden || view.alpha < 0.01) {
        cell.textLabel.textColor = IMLEXColor.deemphasizedTextColor;
        cell.detailTextLabel.textColor = IMLEXColor.deemphasizedTextColor;
    } else {
        cell.textLabel.textColor = IMLEXColor.primaryTextColor;
        cell.detailTextLabel.textColor = IMLEXColor.primaryTextColor;
    }
...
- (void)tableView:(UITableView *)tableView accessoryButtonTappedForRowWithIndexPath:(NSIndexPath *)indexPath {
    ...
    IMLEXObjectExplorerViewController *viewExplorer = [IMLEXObjectExplorerFactory explorerViewControllerForObject:drillInView];
