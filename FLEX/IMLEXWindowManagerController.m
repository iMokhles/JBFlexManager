//
//  IMLEXWindowManagerController.m
//  IMLEX
//
...
@interface IMLEXWindowManagerController ()
...
@implementation IMLEXWindowManagerController
...
- (void)reloadData {
    ...
    self.windowSubtitles = [self.windows IMLEX_mapped:^id(UIWindow *window, NSUInteger idx) {
    ...
    if (@available(iOS 13, *)) {
        ...
        self.sceneSubtitles = [self.scenes IMLEX_mapped:^id(UIScene *scene, NSUInteger idx) {
...
- (void)showRevertOrDismissAlert:(void(^)())revertBlock {
    ...
    } showFrom:[IMLEXUtility topViewControllerInWindow:highestWindow]];
}
...
- (NSString *)sceneDescription:(UIScene *)scene API_AVAILABLE(ios(13)) {
    ...
    if ([scene isKindOfClass:[UIWindowScene class]]) {
        ...
        suffix = IMLEXPluralString(windowScene.windows.count, @"windows", @"window");
    }
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    UIWindow *window = nil;
    NSString *subtitle = nil;
    IMLEXWindow *IMLEX = IMLEXManager.sharedManager.explorerWindow;
    ...
    switch (indexPath.section) {
        ...
        case 2:
            if (@available(iOS 13, *)) {
                UIScene *scene = self.scenes[indexPath.row];
                UIWindowScene *oldScene = IMLEX.windowScene;
                BOOL isWindowScene = [scene isKindOfClass:[UIWindowScene class]];
                BOOL isIMLEXScene = isWindowScene ? IMLEX.windowScene == scene : NO;
                ...
                [IMLEXAlert makeAlert:^(IMLEXAlert *make) {
                    ...
                    if (isWindowScene) {
                        if (isIMLEXScene) {
                            make.message(@"Already the IMLEX window scene");
                        }
                        
                        make.button(@"Set as IMLEX Window Scene")
                        .handler(^(NSArray<NSString *> *strings) {
                            IMLEX.windowScene = (id)scene;
                            [self showRevertOrDismissAlert:^{
                                IMLEX.windowScene = oldScene;
                            }];
                        }).enabled(!isIMLEXScene);
    ...
    subtitle = [subtitle stringByAppendingString:
        @"\n\n1) Adjust the IMLEX window level relative to this window,\n"
        "2) adjust this window's level relative to the IMLEX window,\n"
    ...
    [IMLEXAlert makeAlert:^(IMLEXAlert *make) {
        ...
        make.button(@"Adjust IMLEX Window Level").handler(^(NSArray<NSString *> *strings) {
            targetWindow = IMLEX; oldLevel = IMLEX.windowLevel;
            IMLEX.windowLevel = window.windowLevel + strings.firstObject.integerValue;
        ...
        make.button(@"Adjust This Window's Level").handler(^(NSArray<NSString *> *strings) {
            ...
            window.windowLevel = IMLEX.windowLevel + strings.firstObject.integerValue;
...
- (void)tableView:(UITableView *)tableView accessoryButtonTappedForRowWithIndexPath:(NSIndexPath *)ip {
    [self.navigationController pushViewController:
        [IMLEXObjectExplorerFactory explorerViewControllerForObject:self.sections[ip.section][ip.row]]
    animated:YES];
}
