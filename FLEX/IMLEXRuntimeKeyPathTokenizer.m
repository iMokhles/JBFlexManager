//
//  IMLEXRuntimeKeyPathTokenizer.m
//  IMLEX
//
...
@implementation IMLEXRuntimeKeyPathTokenizer
...
+ (IMLEXRuntimeKeyPath *)tokenizeString:(NSString *)userInput {
    ...
    IMLEXSearchToken *bundle    = [self scanToken:scanner allowed:filenameAllowed first:filenameAllowed];
    IMLEXSearchToken *cls       = [self scanToken:scanner allowed:identifierAllowed first:firstAllowed];
    IMLEXSearchToken *method    = tokens > 2 ? [self scanMethodToken:scanner instance:&instance] : nil;

    return [IMLEXRuntimeKeyPath bundle:bundle
                       class:cls
                      method:method
                  isInstance:instance
                      string:userInput];
...
+ (IMLEXSearchToken *)scanToken:(NSScanner *)scanner allowed:(NSCharacterSet *)allowedChars first:(NSCharacterSet *)first {
    if (scanner.isAtEnd) {
        if ([scanner.string hasSuffix:@"."] && ![scanner.string hasSuffix:@"\\."]) {
            return [IMLEXSearchToken string:nil options:TBWildcardOptionsAny];
        }
        return nil;
    }
    ...
    while (!stop && ![scanner scanString:@"." intoString:&tmp] && !scanner.isAtEnd) {
        ...
        // Scan '\.' or trailing '\'
            ...
            if ([scanner scanString:@"." intoString:nil]) {
                ...
                return [IMLEXSearchToken string:token options:options | TBWildcardOptionsSuffix];
            } else {
    ...
    return [IMLEXSearchToken string:token options:options];
}

+ (IMLEXSearchToken *)scanMethodToken:(NSScanner *)scanner instance:(NSNumber **)instance {
    if (scanner.isAtEnd) {
    ...
    if (scanner.isAtEnd) {
        return [IMLEXSearchToken string:@"" options:TBWildcardOptionsSuffix];
    }
