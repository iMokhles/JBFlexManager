...
#import "IMLEXWindowManagerController.h"
#import "IMLEXViewControllersViewController.h"
#import "NSUserDefaults+IMLEX.h"

typedef NS_ENUM(NSUInteger, IMLEXExplorerMode) {
    IMLEXExplorerModeDefault,
    IMLEXExplorerModeSelect,
    IMLEXExplorerModeMove
};

@interface IMLEXExplorerViewController () <IMLEXHierarchyDelegate, UIAdaptivePresentationControllerDelegate>

/// Tracks the currently active tool/mode
@property (nonatomic) IMLEXExplorerMode currentMode;
...
/// self.view.window as a \c IMLEXWindow
@property (nonatomic, readonly) IMLEXWindow *window;
...
@implementation IMLEXExplorerViewController
...
- (void)viewDidLoad {
    ...
    // Toolbar
    _explorerToolbar = [IMLEXExplorerToolbar new];

    // Start the toolbar off below any bars that may be at the top of the view.
    CGFloat toolbarOriginY = NSUserDefaults.standardUserDefaults.IMLEX_toolbarTopMargin;
    ...
    self.movePanGR.enabled = self.currentMode == IMLEXExplorerModeMove;
...
- (UIViewController *)viewControllerForRotationAndOrientation {
    UIViewController *viewController = IMLEXUtility.appKeyWindow.rootViewController;
...
//    }
    ...
    UIInterfaceOrientationMask supportedOrientations = IMLEXUtility.infoPlistSupportedInterfaceOrientationsMask;
...
- (void)viewWillTransitionToSize:(CGSize)size
    ...
    [coordinator animateAlongsideTransition:^(id<UIViewControllerTransitionCoordinatorContext> context) {
        ...
        for (UIView *view in self.viewsAtTapPoint) {
            ...
            if (self.currentMode == IMLEXExplorerModeSelect) {
...
- (void)setSelectedView:(UIView *)selectedView {
    if (![_selectedView isEqual:selectedView]) {
        ...
        self.explorerToolbar.selectedViewDescription = [IMLEXUtility
            descriptionForView:selectedView includingFrame:YES
        ];
        self.explorerToolbar.selectedViewOverlayColor = [IMLEXUtility
        ...
        if (selectedView) {
            ...
            UIColor *outlineColor = [IMLEXUtility consistentRandomColorForObject:selectedView];
...
- (void)setCurrentMode:(IMLEXExplorerMode)currentMode {
    if (_currentMode != currentMode) {
        _currentMode = currentMode;
        switch (currentMode) {
            case IMLEXExplorerModeDefault:
                [self removeAndClearOutlineViews];
                self.viewsAtTapPoint = nil;
                self.selectedView = nil;
                break;

            case IMLEXExplorerModeSelect:
        ...
        self.movePanGR.enabled = currentMode == IMLEXExplorerModeMove;
        [self updateButtonStates];
    }
}
...
- (void)setupToolbarActions {
    IMLEXExplorerToolbar *toolbar = self.explorerToolbar;
    NSDictionary<NSString *, IMLEXExplorerToolbarItem *> *actionsToItems = @{
    ...
    [actionsToItems enumerateKeysAndObjectsUsingBlock:^(NSString *sel, IMLEXExplorerToolbarItem *item, BOOL *stop) {
        [item addTarget:self action:NSSelectorFromString(sel) forControlEvents:UIControlEventTouchUpInside];
    }];
}

- (void)selectButtonTapped:(IMLEXExplorerToolbarItem *)sender {
    [self toggleSelectTool];
}

- (void)hierarchyButtonTapped:(IMLEXExplorerToolbarItem *)sender {
...
- (void)recentButtonTapped:(IMLEXExplorerToolbarItem *)sender {
    NSAssert(IMLEXTabList.sharedList.activeTab, @"Must have active tab");
    [self presentViewController:IMLEXTabList.sharedList.activeTab animated:YES completion:nil];
}

- (void)moveButtonTapped:(IMLEXExplorerToolbarItem *)sender {
    [self toggleMoveTool];
}

- (void)globalsButtonTapped:(IMLEXExplorerToolbarItem *)sender {
    [self toggleMenuTool];
}

- (void)closeButtonTapped:(IMLEXExplorerToolbarItem *)sender {
    self.currentMode = IMLEXExplorerModeDefault;
...
- (void)updateButtonStates {
    ...
    toolbar.selectItem.selected = self.currentMode == IMLEXExplorerModeSelect;
    ...
    toolbar.moveItem.selected = self.currentMode == IMLEXExplorerModeMove;

    // Recent only enabled when we have a last active tab
    toolbar.recentItem.enabled = IMLEXTabList.sharedList.activeTab != nil;
}
...
- (void)updateToolbarPositionWithUnconstrainedFrame:(CGRect)unconstrainedFrame {
    ...
    NSUserDefaults.standardUserDefaults.IMLEX_toolbarTopMargin = unconstrainedFrame.origin.y;
}
...
- (void)handleToolbarDetailsTapGesture:(UITapGestureRecognizer *)tapGR {
    if (tapGR.state == UIGestureRecognizerStateRecognized && self.selectedView) {
        UIViewController *topStackVC = [IMLEXObjectExplorerFactory explorerViewControllerForObject:self.selectedView];
        [self presentViewController:
            [IMLEXNavigationController withRootViewController:topStackVC]
...
- (void)handleToolbarShowTabsGesture:(UILongPressGestureRecognizer *)sender {
    if (sender.state == UIGestureRecognizerStateBegan) {
        ...
        // Don't use IMLEXNavigationController because the tab viewer itself is not a tab
        [super presentViewController:[[UINavigationController alloc]
            initWithRootViewController:[IMLEXTabsViewController new]
        ] animated:YES completion:nil];
    }
}
...
- (void)handleToolbarWindowManagerGesture:(UILongPressGestureRecognizer *)sender {
    if (sender.state == UIGestureRecognizerStateBegan) {
        ...
        [super presentViewController:[IMLEXNavigationController
            withRootViewController:[IMLEXWindowManagerController new]
        ] animated:YES completion:nil];
    }
}
...
- (void)handleToolbarShowViewControllersGesture:(UILongPressGestureRecognizer *)sender {
    if (sender.state == UIGestureRecognizerStateBegan && self.viewsAtTapPoint.count) {
        ...
        UIViewController *list = [IMLEXViewControllersViewController
            controllersForViews:self.viewsAtTapPoint
        ];
        [self presentViewController:
            [IMLEXNavigationController withRootViewController:list
        ] animated:YES completion:nil];
    }
}
...
- (void)handleSelectionTap:(UITapGestureRecognizer *)tapGR {
    ...
    if (self.currentMode == IMLEXExplorerModeSelect && tapGR.state == UIGestureRecognizerStateRecognized) {
        // Note that [tapGR locationInView:nil] is broken in iOS 8,
        // so we have to do a two step conversion to window coordinates.
        // Thanks to @lascorbe for finding this: https://github.com/Flipboard/IMLEX/pull/31
...
- (UIView *)outlineViewForView:(UIView *)view {
    ...
    outlineView.layer.borderColor = [IMLEXUtility consistentRandomColorForObject:view].CGColor;
...
- (NSArray<UIView *> *)viewsAtPoint:(CGPoint)tapPointInWindow skipHiddenViews:(BOOL)skipHidden {
    ...
    for (UIWindow *window in IMLEXUtility.allWindows) {
...
- (UIView *)viewForSelectionAtPoint:(CGPoint)tapPointInWindow {
    ...
    for (UIWindow *window in IMLEXUtility.allWindows.reverseObjectEnumerator) {
...
- (void)updateSelectedViewPositionWithDragGesture:(UIPanGestureRecognizer *)movePanGR {
    ...
    newSelectedViewFrame.origin.x = IMLEXFloor(newSelectedViewFrame.origin.x + translation.x);
    newSelectedViewFrame.origin.y = IMLEXFloor(newSelectedViewFrame.origin.y + translation.y);
...
- (BOOL)shouldReceiveTouchAtWindowPoint:(CGPoint)pointInWindowCoordinates {
    ...
    if (!shouldReceiveTouch && self.currentMode == IMLEXExplorerModeSelect) {
        shouldReceiveTouch = YES;
    }

    // Always in move mode too
    if (!shouldReceiveTouch && self.currentMode == IMLEXExplorerModeMove) {
        shouldReceiveTouch = YES;
    }
...
#pragma mark - IMLEXHierarchyDelegate
...
- (void)viewHierarchyDidDismiss:(UIView *)selectedView {
    ...
    [self toggleViewsToolWithCompletion:^{
        ...
        if (self.currentMode == IMLEXExplorerModeDefault && selectedView) {
            self.currentMode = IMLEXExplorerModeSelect;
        }
...
- (void)presentViewController:(UIViewController *)toPresent
    ...
    // Move the status bar on top of IMLEX so we can get scroll to top behavior for taps.
    if (!@available(iOS 13, *)) {
...
- (void)dismissViewControllerAnimated:(BOOL)animated completion:(void (^)(void))completion {
    ...
    // We want it above IMLEX while a modal is presented for
    // scroll to top, but below IMLEX otherwise for exploration.
...
- (void)toggleMoveTool {
    if (self.currentMode == IMLEXExplorerModeMove) {
        self.currentMode = IMLEXExplorerModeSelect;
    } else if (self.currentMode == IMLEXExplorerModeSelect && self.selectedView) {
        self.currentMode = IMLEXExplorerModeMove;
    }
}
...
- (void)toggleViewsToolWithCompletion:(void(^)(void))completion {
    [self toggleToolWithViewControllerProvider:^UINavigationController *{
        if (self.selectedView) {
            return [IMLEXHierarchyViewController
                delegate:self
                viewsAtTap:self.viewsAtTapPoint
                selectedView:self.selectedView
            ];
        } else {
            return [IMLEXHierarchyViewController delegate:self];
        }
    } completion:^{
        if (completion) {
            completion();
...
- (void)toggleMenuTool {
    [self toggleToolWithViewControllerProvider:^UINavigationController *{
        return [IMLEXNavigationController withRootViewController:[IMLEXGlobalsViewController new]];
    } completion:nil];
}
...
- (BOOL)handleDownArrowKeyPressed {
    ...
    } else if (self.currentMode == IMLEXExplorerModeSelect && self.viewsAtTapPoint.count > 0) {
