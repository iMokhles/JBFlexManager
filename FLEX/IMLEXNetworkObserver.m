...
NSString *const kIMLEXNetworkObserverEnabledStateChangedNotification = @"kIMLEXNetworkObserverEnabledStateChangedNotification";
static NSString *const kIMLEXNetworkObserverEnabledDefaultsKey = @"com.IMLEX.IMLEXNetworkObserver.enableOnLaunch";
...
@interface IMLEXInternalRequestState : NSObject
...
@implementation IMLEXInternalRequestState

@end

@interface IMLEXNetworkObserver (NSURLConnectionHelpers)
...
@interface IMLEXNetworkObserver (NSURLSessionTaskHelpers)
...
@interface IMLEXNetworkObserver ()

@property (nonatomic) NSMutableDictionary<NSString *, IMLEXInternalRequestState *> *requestStatesForRequestIDs;
@property (nonatomic) dispatch_queue_t queue;

@end

@implementation IMLEXNetworkObserver
...
+ (void)setEnabled:(BOOL)enabled {
    ...
    [NSUserDefaults.standardUserDefaults setBool:enabled forKey:kIMLEXNetworkObserverEnabledDefaultsKey];
    ...
    if (previouslyEnabled != enabled) {
        [NSNotificationCenter.defaultCenter postNotificationName:kIMLEXNetworkObserverEnabledStateChangedNotification object:self];
    }
}

+ (BOOL)isEnabled {
    return [[NSUserDefaults.standardUserDefaults objectForKey:kIMLEXNetworkObserverEnabledDefaultsKey] boolValue];
}
...
+ (instancetype)sharedObserver {
    static IMLEXNetworkObserver *sharedObserver = nil;
...
+ (void)sniffWithoutDuplicationForObject:(NSObject *)object selector:(SEL)selector sniffingBlock:(void (^)(void))sniffingBlock originalImplementationBlock:(void (^)(void))originalImplementationBlock {
    ...
    // See https://github.com/Flipboard/IMLEX/issues/61 for an example.
    if (!object) {
    ...
    dispatch_once(&onceToken, ^{
        ...
        if (numClasses > 0) {
            ...
            for (NSInteger classIndex = 0; classIndex < numClasses; ++classIndex) {
                ...
                if (class == [IMLEXNetworkObserver class]) {
                    continue;
                }
...
+ (void)injectIntoNSURLConnectionCancel {
    ...
    dispatch_once(&onceToken, ^{
        ...
        SEL swizzledSelector = [IMLEXUtility swizzledSelectorForSelector:selector];
        Method originalCancel = class_getInstanceMethod(class, selector);

        void (^swizzleBlock)(NSURLConnection *) = ^(NSURLConnection *slf) {
            [IMLEXNetworkObserver.sharedObserver connectionWillCancel:slf];
...
+ (void)injectIntoNSURLSessionTaskResume {
    ...
    dispatch_once(&onceToken, ^{
        ...
        if (sel_af_resume) {
            NSMutableArray<Class> *classTree = IMLEXGetAllSubclasses(baseResumeClass, NO).mutableCopy;
            for (NSInteger i = 0; i < classTree.count; i++) {
                [classTree addObjectsFromArray:IMLEXGetAllSubclasses(classTree[i], NO)];
            }
...
+ (void)swizzleResumeSelector:(SEL)selector forClass:(Class)class {
    ...
    IMP implementation = imp_implementationWithBlock(^(NSURLSessionTask *slf) {
        ...
        [IMLEXNetworkObserver.sharedObserver URLSessionTaskWillResume:slf];
        ((void(*)(id, SEL))objc_msgSend)(
            slf, swizzledSelector
        );
    });
    ...
    dispatch_once(&onceToken, ^{
        ...
        SendAsyncRequestBlock swizzleBlock = ^(Class slf,
            ...
            if (IMLEXNetworkObserver.isEnabled) {
                ...
                [IMLEXNetworkRecorder.defaultRecorder
                ...
                [IMLEXNetworkRecorder.defaultRecorder recordMechanism:mechanism forRequestID:requestID];
        ...
        [IMLEXUtility replaceImplementationOfKnownSelector:selector
...
+ (void)injectIntoNSURLSessionAsyncDataAndDownloadTaskMethods {
    ...
    dispatch_once(&onceToken, ^{
        ...
        for (int selectorIndex = 0; selectorIndex < numSelectors; selectorIndex++) {
            ...
            if ([IMLEXUtility instanceRespondsButDoesNotImplementSelector:selector class:class]) {
            ...
            NSURLSessionNewTaskMethod swizzleBlock = ^NSURLSessionTask *(NSURLSession *slf,
                ...
                if (IMLEXNetworkObserver.isEnabled && completion) {
                    NSString *requestID = [self nextRequestID];
                    NSString *mechanism = [self mechanismFromClassMethod:selector onClass:class];
...
+ (void)injectWillSendRequestIntoDelegateClass:(Class)cls {
    ...
    WillSendRequestBlock undefinedBlock = ^NSURLRequest *(id slf,
                                                          ...
                                                          NSURLRequest *request,
                                                          NSURLResponse *response) {
        [IMLEXNetworkObserver.sharedObserver
    ...
    [IMLEXUtility replaceImplementationOfSelector:selector
...
+ (void)injectDidReceiveResponseIntoDelegateClass:(Class)cls {
    ...
    DidReceiveResponseBlock undefinedBlock = ^(id<NSURLConnectionDelegate> slf,
                                               NSURLConnection *connection,
                                               NSURLResponse *response) {
        [IMLEXNetworkObserver.sharedObserver connection:connection
...
+ (void)injectDidFinishLoadingIntoDelegateClass:(Class)cls {
    ...
    FinishLoadingBlock undefinedBlock = ^(id<NSURLConnectionDelegate> slf, NSURLConnection *connection) {
        [IMLEXNetworkObserver.sharedObserver connectionDidFinishLoading:connection delegate:slf];
    };
...
+ (void)injectTaskDidReceiveDataIntoDelegateClass:(Class)cls {
    ...
    DidReceiveDataBlock undefinedBlock = ^(id<NSURLSessionDataDelegate> slf,
                                           ...
                                           NSURLSessionDataTask *dataTask,
                                           NSData *data) {
        [IMLEXNetworkObserver.sharedObserver URLSession:session
...
static char const * const kIMLEXRequestIDKey = "kIMLEXRequestIDKey";

+ (NSString *)requestIDForConnectionOrTask:(id)connectionOrTask {
    NSString *requestID = objc_getAssociatedObject(connectionOrTask, kIMLEXRequestIDKey);
    if (!requestID) {
...
+ (void)setRequestID:(NSString *)requestID forConnectionOrTask:(id)connectionOrTask {
    objc_setAssociatedObject(
        connectionOrTask, kIMLEXRequestIDKey, requestID, OBJC_ASSOCIATION_RETAIN_NONATOMIC
    );
}
...
- (id)init {
    ...
    if (self) {
        ...
        self.queue = dispatch_queue_create(
            "com.IMLEX.IMLEXNetworkObserver", DISPATCH_QUEUE_SERIAL
        );
    }
...
- (IMLEXInternalRequestState *)requestStateForRequestID:(NSString *)requestID {
    IMLEXInternalRequestState *requestState = self.requestStatesForRequestIDs[requestID];
    if (!requestState) {
        requestState = [IMLEXInternalRequestState new];
...
@implementation IMLEXNetworkObserver (NSURLConnectionHelpers)
...
- (void)connection:(NSURLConnection *)connection
  ...
  redirectResponse:(NSURLResponse *)response
    ...
    [self performBlock:^{
        ...
        IMLEXInternalRequestState *requestState = [self requestStateForRequestID:requestID];
...
@implementation IMLEXNetworkObserver (NSURLSessionTaskHelpers)
