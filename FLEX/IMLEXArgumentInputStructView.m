//
//  IMLEXArgumentInputStructView.m
//  Flipboard
//
...
@interface IMLEXArgumentInputStructView ()

@property (nonatomic) NSArray<IMLEXArgumentInputView *> *argumentInputViews;

@end

@implementation IMLEXArgumentInputStructView

- (instancetype)initWithArgumentTypeEncoding:(const char *)typeEncoding {
    self = [super initWithArgumentTypeEncoding:typeEncoding];
    if (self) {
        NSMutableArray<IMLEXArgumentInputView *> *inputViews = [NSMutableArray new];
        NSArray<NSString *> *customTitles = [[self class] customFieldTitlesForTypeEncoding:typeEncoding];
        [IMLEXRuntimeUtility enumerateTypesInStructEncoding:typeEncoding usingBlock:^(NSString *structName,
                                                                                     const char *fieldTypeEncoding,
                                                                                     NSString *prettyTypeEncoding,
                                                                                     NSUInteger fieldIndex,
                                                                                     NSUInteger fieldOffset) {
            
            IMLEXArgumentInputView *inputView = [IMLEXArgumentInputViewFactory argumentInputViewForTypeEncoding:fieldTypeEncoding];
            inputView.targetSize = IMLEXArgumentInputViewSizeSmall;
...
- (void)setBackgroundColor:(UIColor *)backgroundColor {
    ...
    for (IMLEXArgumentInputView *inputView in self.argumentInputViews) {
...
- (void)setInputValue:(id)inputValue {
    if ([inputValue isKindOfClass:[NSValue class]]) {
        ...
        if (strcmp(self.typeEncoding.UTF8String, structTypeEncoding) == 0) {
            ...
            if (IMLEXGetSizeAndAlignment(structTypeEncoding, &valueSize, NULL)) {
                void *unboxedValue = malloc(valueSize);
                [inputValue getValue:unboxedValue];
                [IMLEXRuntimeUtility enumerateTypesInStructEncoding:structTypeEncoding usingBlock:^(NSString *structName,
                                                                                                   const char *fieldTypeEncoding,
                                                                                                   NSString *prettyTypeEncoding,
                                                                                                   NSUInteger fieldIndex,
                                                                                                   NSUInteger fieldOffset) {
                    ...
                    IMLEXArgumentInputView *inputView = self.argumentInputViews[fieldIndex];
                    
                    if (fieldTypeEncoding[0] == IMLEXTypeEncodingObjcObject || fieldTypeEncoding[0] == IMLEXTypeEncodingObjcClass) {
                        inputView.inputValue = (__bridge id)fieldPointer;
                    } else {
                        NSValue *boxedField = [IMLEXRuntimeUtility valueForPrimitivePointer:fieldPointer objCType:fieldTypeEncoding];
...
- (id)inputValue {
    ...
    if (IMLEXGetSizeAndAlignment(structTypeEncoding, &structSize, NULL)) {
...
+ (BOOL)supportsObjCType:(const char *)type withCurrentValue:(id)value {
    ...
    if (type[0] == IMLEXTypeEncodingStructBegin) {
        return IMLEXGetSizeAndAlignment(type, nil, nil);
    }
