...
@interface IMLEXShortcutsSection ()
...
@property (nonatomic, copy) NSArray<id<IMLEXShortcut>> *shortcuts;
@property (nonatomic, readonly) NSArray<id<IMLEXShortcut>> *allShortcuts;
@end

@implementation IMLEXShortcutsSection
...
+ (instancetype)forObject:(id)objectOrClass additionalRows:(NSArray *)toPrepend {
    NSArray *rows = [IMLEXShortcutsFactory shortcutsForObjectOrClass:objectOrClass];
...
- (id)initWithObject:object rows:(NSArray *)rows {
    ...
    if (self) {
        ...
        _allShortcuts = [rows IMLEX_mapped:^id(id obj, NSUInteger idx) {
            return [IMLEXShortcut shortcutFor:obj];
        }];
        _numberOfLines = 1;
...
- (void)setFilterText:(NSString *)filterText {
    ...
    if (filterText.length) {
        ...
        [self.allTitles IMLEX_forEach:filterBlock];
        [self.allSubtitles IMLEX_forEach:filterBlock];
        ...
        self.subtitles = [self.allSubtitles IMLEX_filtered:^BOOL(NSString *sub, NSUInteger idx) {
            return sub.length > 0;
        }];
    }
}
...
- (void)reloadData {
    ...
    if (self.allShortcuts) {
        self.allTitles = [self.allShortcuts IMLEX_mapped:^id(IMLEXShortcut *s, NSUInteger idx) {
            return [s titleWith:self.object];
        }];
        self.allSubtitles = [self.allShortcuts IMLEX_mapped:^id(IMLEXShortcut *s, NSUInteger idx) {
...
- (void (^)(__kindof UIViewController *))didPressInfoButtonAction:(NSInteger)row {
    id<IMLEXShortcut> shortcut = self.shortcuts[row];
...
- (NSString *)reuseIdentifierForRow:(NSInteger)row {
    IMLEXTableViewCellReuseIdentifier defaultReuse = kIMLEXDetailCell;
    if (@available(iOS 11, *)) {
        defaultReuse = kIMLEXMultilineDetailCell;
    }
...
@interface IMLEXShortcutsFactory () {
    BOOL _append, _prepend, _replace, _notInstance;
    NSArray<NSString *> *_properties, *_ivars, *_methods;
}
@end

#define NewAndSet(ivar) ({ IMLEXShortcutsFactory *r = [self new]; r->ivar = YES; r; })
#define SetIvar(ivar) ({ self->ivar = YES; self; })
#define SetParamBlock(ivar) ^(NSArray *p) { self->ivar = p; return self; }

@implementation IMLEXShortcutsFactory

typedef NSMutableDictionary<Class, NSMutableArray<id<IMLEXRuntimeMetadata>> *> RegistrationBuckets;
// Class buckets
...
+ (NSArray<id<IMLEXRuntimeMetadata>> *)shortcutsForObjectOrClass:(id)objectOrClass {
    NSMutableArray<id<IMLEXRuntimeMetadata>> *shortcuts = [NSMutableArray new];
    ...
    for (id<IMLEXRuntimeMetadata> metadata in shortcuts) {
...
+ (IMLEXShortcutsFactory *)append {
    return NewAndSet(_append);
}

+ (IMLEXShortcutsFactory *)prepend {
    return NewAndSet(_prepend);
}

+ (IMLEXShortcutsFactory *)replace {
    return NewAndSet(_replace);
}

- (void)_register:(NSArray<id<IMLEXRuntimeMetadata>> *)items to:(RegistrationBuckets *)global class:(Class)key {
...
- (IMLEXShortcutsFactoryTarget)forClass {
    return ^(Class cls) {
        ...
        if (self->_properties) {
            NSArray *items = [self->_properties IMLEX_mapped:^id(NSString *name, NSUInteger idx) {
                return [IMLEXProperty named:name onClass:clsForMetadata];
            }];
            [self _register:items to:propertyBucket class:cls];
        }

        if (self->_methods) {
            NSArray *items = [self->_methods IMLEX_mapped:^id(NSString *name, NSUInteger idx) {
                return [IMLEXMethod selector:NSSelectorFromString(name) class:clsForMetadata];
            }];
        ...
        if (self->_ivars) {
            ...
            NSArray *items = [self->_ivars IMLEX_mapped:^id(NSString *name, NSUInteger idx) {
                return [IMLEXIvar named:name onClass:clsForMetadata];
            }];
