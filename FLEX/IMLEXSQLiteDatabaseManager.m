...
@interface IMLEXSQLiteDatabaseManager ()
@property (nonatomic) sqlite3 *db;
@property (nonatomic, copy) NSString *path;
@end

@implementation IMLEXSQLiteDatabaseManager

#pragma mark - IMLEXDatabaseManager
...
#if SQLITE_HAS_CODEC
    NSString *defaultSqliteDatabasePassword = IMLEXManager.sharedManager.defaultSqliteDatabasePassword;
...
- (NSArray<NSString *> *)queryAllTables {
    return [[self executeStatement:QUERY_TABLENAMES].rows IMLEX_mapped:^id(NSArray *table, NSUInteger idx) {
...
- (NSArray<NSString *> *)queryAllColumnsOfTable:(NSString *)tableName {
    ...
    IMLEXSQLResult *results = [self executeStatement:sql];
    
    return [results.keyedRows IMLEX_mapped:^id(NSDictionary *column, NSUInteger idx) {
        return column[@"name"];
    }];
}
...
- (IMLEXSQLResult *)executeStatement:(NSString *)sql {
    [self open];
    
    IMLEXSQLResult *result = nil;
    ...
    if (sqlite3_prepare_v2(_db, sql.UTF8String, -1, &pstmt, 0) == SQLITE_OK) {
        ...
        NSArray<NSString *> *columns = [NSArray IMLEX_forEachUpTo:columnCount map:^id(NSUInteger i) {
        ...
        while ((status = sqlite3_step(pstmt)) == SQLITE_ROW) {
            ...
            if (dataCount > 0) {
                [rows addObject:[NSArray IMLEX_forEachUpTo:columnCount map:^id(NSUInteger i) {
        ...
        if (status == SQLITE_DONE) {
            if (rows.count) {
                ...
                result = [IMLEXSQLResult columns:columns rows:rows];
            } else {
                ...
                result = [IMLEXSQLResult message:message];
            }
        } else {
            // An error occured executing the query
            result = [IMLEXSQLResult message:@(sqlite3_errmsg(_db) ?: "(Execution: empty error)")];
        }
    } else {
        // An error occurred creating the prepared statement
        result = [IMLEXSQLResult message:@(sqlite3_errmsg(_db) ?: "(Prepared statement: empty error)")];
    }
