...
@interface IMLEXOSLogController ()

+ (IMLEXOSLogController *)sharedLogController;

@property (nonatomic) void (^updateHandler)(NSArray<IMLEXSystemLogMessage *> *);
...
@implementation IMLEXOSLogController

+ (void)load {
    // Persist logs when the app launches on iOS 10 if we have persistent logs turned on
    if (IMLEXOSLogAvailable()) {
        if (NSUserDefaults.standardUserDefaults.IMLEX_cacheOSLogMessages) {
...
+ (instancetype)sharedLogController {
    static IMLEXOSLogController *shared = nil;
...
+ (instancetype)withUpdateHandler:(void(^)(NSArray<IMLEXSystemLogMessage *> *newMessages))newMessagesHandler {
    IMLEXOSLogController *shared = [self sharedLogController];
    shared.updateHandler = newMessagesHandler;
    return shared;
}

- (id)init {
    NSAssert(IMLEXOSLogAvailable(), @"os_log is only available on iOS 10 and up");
...
- (BOOL)handleStreamEntry:(os_activity_stream_entry_t)entry error:(int)error {
    ...
    if (!error && entry) {
        if (entry->type == OS_ACTIVITY_STREAM_TYPE_LOG_MESSAGE ||
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                IMLEXSystemLogMessage *message = [IMLEXSystemLogMessage logMessageFromDate:date text:msg];
                if (self.persistent) {
