...
@interface IMLEXWebViewController () <WKNavigationDelegate>
...
@implementation IMLEXWebViewController
...
- (id)initWithText:(NSString *)text {
    ...
    if (self) {
        ...
        NSString *htmlString = [NSString stringWithFormat:@"<head><meta name='viewport' content='initial-scale=1.0'></head><body><pre>%@</pre></body>", [IMLEXUtility stringByEscapingHTMLEntitiesInString:text]];
...
- (void)webView:(WKWebView *)webView decidePolicyForNavigationAction:(WKNavigationAction *)navigationAction decisionHandler:(void (^)(WKNavigationActionPolicy))decisionHandler {
    ...
    if (navigationAction.navigationType == WKNavigationTypeOther) {
        ...
        IMLEXWebViewController *webVC = [[[self class] alloc] initWithURL:[request URL]];
