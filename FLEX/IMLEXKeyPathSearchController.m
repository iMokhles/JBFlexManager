//
//  IMLEXKeyPathSearchController.m
//  IMLEX
//
...
@interface IMLEXKeyPathSearchController ()
@property (nonatomic, readonly, weak) id<IMLEXKeyPathSearchControllerDelegate> delegate;
...
@property (nonatomic) IMLEXRuntimeKeyPath *keyPath;
...
@property (nonatomic) NSArray<NSArray<IMLEXMethod *> *> *classesToMethods;
@end

@implementation IMLEXKeyPathSearchController

+ (instancetype)delegate:(id<IMLEXKeyPathSearchControllerDelegate>)delegate {
    IMLEXKeyPathSearchController *controller = [self new];
    controller->_bundlesOrClasses = [IMLEXRuntimeController allBundleNames];
...
- (void)setToolbar:(IMLEXRuntimeBrowserToolbar *)toolbar {
    _toolbar = toolbar;
...
- (void)didSelectKeyPathOption:(NSString *)text {
    ...
    self.keyPath = [IMLEXRuntimeKeyPathTokenizer tokenizeString:keyPath];
...
- (NSArray<NSString *> *)suggestions {
    if (self.bundlesOrClasses) {
        if (self.classes) {
            ...
            return [self.filteredClasses IMLEX_subArrayUpto:10];
        }
        ...
        return [self.bundlesOrClasses IMLEX_subArrayUpto:10];
    }
...
- (void)updateTable {
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_HIGH, 0), ^{
        if (self.classes) {
            ...
            NSMutableArray *methods = [IMLEXRuntimeController
            ...
            IMLEXRuntimeKeyPath *keyPath = self.keyPath;
            NSArray *models = [IMLEXRuntimeController dataForKeyPath:keyPath];
            if (keyPath.methodKey) { // We're looking at methods
                self.bundlesOrClasses = nil;
                
                NSMutableArray *methods = models.mutableCopy;
                NSMutableArray<NSString *> *classes = [
                    IMLEXRuntimeController classesForKeyPath:keyPath
                ];
...
- (void)setNonEmptyMethodLists:(NSMutableArray<NSArray<IMLEXMethod *> *> *)methods
...
- (BOOL)searchBar:(UISearchBar *)searchBar shouldChangeTextInRange:(NSRange)range replacementText:(NSString *)text {
    ...
    if (![IMLEXRuntimeKeyPathTokenizer allowedInKeyPath:text]) {
        return NO;
    }
    ...
    @try {
        ...
        self.keyPath = [IMLEXRuntimeKeyPathTokenizer tokenizeString:text];
...
- (void)searchBar:(UISearchBar *)searchBar textDidChange:(NSString *)searchText {
    ...
    // ... or remove all rows
        ...
        _bundlesOrClasses = [IMLEXRuntimeController allBundleNames];
...
- (void)searchBarCancelButtonClicked:(UISearchBar *)searchBar {
    self.keyPath = IMLEXRuntimeKeyPath.empty;
    [self updateTable];
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    UITableViewCell *cell = [tableView
        dequeueReusableCellWithIdentifier:kIMLEXMultilineDetailCell
    ...
    // One row per section
        ...
        NSArray<IMLEXMethod *> *methods = self.classesToMethods[indexPath.row];
        NSMutableString *summary = [NSMutableString new];
        [methods enumerateObjectsUsingBlock:^(IMLEXMethod *method, NSUInteger idx, BOOL *stop) {
            NSString *format = nil;
...
- (NSString *)tableView:(UITableView *)tableView titleForHeaderInSection:(NSInteger)section {
    if (self.filteredClasses || self.keyPath.methodKey) {
        ...
        if (self.keyPath.classKey) {
            return IMLEXPluralString(count, @"classes", @"class");
        } else {
            return IMLEXPluralString(count, @"bundles", @"bundle");
        }
    }
...
- (void)tableView:(UITableView *)tableView accessoryButtonTappedForRowWithIndexPath:(NSIndexPath *)indexPath {
    NSString *bundleSuffixOrClass = self.bundlesOrClasses[indexPath.row];
    NSString *imagePath = [IMLEXRuntimeController imagePathWithShortName:bundleSuffixOrClass];
