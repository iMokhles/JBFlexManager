...
typedef NS_ENUM(NSUInteger, IMLEXArgumentInputViewSize) {
    /// 2 lines, medium-sized
    IMLEXArgumentInputViewSizeDefault = 0,
    /// One line
    IMLEXArgumentInputViewSizeSmall,
    /// Several lines
    IMLEXArgumentInputViewSizeLarge
};

@protocol IMLEXArgumentInputViewDelegate;

@interface IMLEXArgumentInputView : UIView
...
@property (nonatomic) IMLEXArgumentInputViewSize targetSize;

/// Users of the input view can get delegate callbacks for incremental changes in user input.
@property (nonatomic, weak) id <IMLEXArgumentInputViewDelegate> delegate;
...
/// Used by IMLEXArgumentInputViewFactory to create appropriate input views.
...
@protocol IMLEXArgumentInputViewDelegate <NSObject>
