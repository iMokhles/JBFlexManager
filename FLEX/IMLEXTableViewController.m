...
CGFloat const kIMLEXDebounceInstant = 0.f;
CGFloat const kIMLEXDebounceFast = 0.05;
CGFloat const kIMLEXDebounceForAsyncSearch = 0.15;
CGFloat const kIMLEXDebounceForExpensiveIO = 0.5;

@interface IMLEXTableViewController ()
...
@implementation IMLEXTableViewController
@dynamic tableView;
...
- (id)initWithStyle:(UITableViewStyle)style {
    ...
    if (self) {
        _searchBarDebounceInterval = kIMLEXDebounceFast;
...
- (void)setShowsCarousel:(BOOL)showsCarousel {
    ...
    if (showsCarousel) {
        _carousel = ({
            ...
            IMLEXScopeCarousel *carousel = [IMLEXScopeCarousel new];
            ...
            [carousel registerBlockForDynamicTypeChanges:^(IMLEXScopeCarousel *carousel) {
                __typeof(self) self = weakSelf;
...
- (void)loadView {
    self.view = [IMLEXTableView style:self.style];
    self.tableView.dataSource = self;
    self.tableView.delegate = self;

    _shareToolbarItem = IMLEXBarButtonItemSystem(Action, self, @selector(shareButtonPressed));
    _bookmarksToolbarItem = [UIBarButtonItem
        itemWithImage:IMLEXResources.bookmarksIcon target:self action:@selector(showBookmarks)
    ];
    _openTabsToolbarItem = [UIBarButtonItem
        itemWithImage:IMLEXResources.openTabsIcon target:self action:@selector(showTabSwitcher)
    ];

    self.leftmostToolbarItem = UIBarButtonItem.IMLEX_fixedSpace;
    self.middleLeftToolbarItem = UIBarButtonItem.IMLEX_fixedSpace;
    self.middleToolbarItem = UIBarButtonItem.IMLEX_fixedSpace;
}
...
- (void)setupToolbarItems {
    ...
    // Disable tabs entirely when not presented by IMLEXExplorerViewController
    UIViewController *presenter = self.navigationController.presentingViewController;
    if (![presenter isKindOfClass:[IMLEXExplorerViewController class]]) {
...
- (void)layoutTableHeaderIfNeeded {
    if (self.showsCarousel) {
        self.carousel.frame = IMLEXRectSetHeight(
...
- (void)addCarousel:(IMLEXScopeCarousel *)carousel {
    if (@available(iOS 11.0, *)) {
        ...
        if (self.showsSearchBar) {
            carousel.frame = subviewFrame = IMLEXRectSetY(
...
- (void)removeCarousel:(IMLEXScopeCarousel *)carousel {
...
- (void)addSearchController:(UISearchController *)controller {
    if (@available(iOS 11.0, *)) {
        ...
        if (self.showsCarousel) {
            self.carousel.frame = IMLEXRectSetY(
...
- (void)removeSearchController:(UISearchController *)controller {
    if (@available(iOS 11.0, *)) {
        ...
        if (self.showsCarousel) {
//            self.carousel.frame = IMLEXRectRemake(CGPointZero, self.carousel.frame.size);
...
- (void)showBookmarks {
    UINavigationController *nav = [[UINavigationController alloc]
        initWithRootViewController:[IMLEXBookmarksViewController new]
    ];
...
- (void)updateSearchResultsForSearchController:(UISearchController *)searchController {
    ...
    if (text.length && self.searchBarDebounceInterval > kIMLEXDebounceInstant) {
