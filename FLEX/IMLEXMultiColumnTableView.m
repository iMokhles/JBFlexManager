...
@interface IMLEXMultiColumnTableView () <
...
@implementation IMLEXMultiColumnTableView
...
- (instancetype)initWithFrame:(CGRect)frame {
    ...
    if (self) {
        ...
        self.backgroundColor  = IMLEXColor.groupedBackgroundColor;
...
- (void)loadHeaderScrollView {
    ...
    headerScrollView.backgroundColor = IMLEXColor.secondaryGroupedBackgroundColor;
...
- (void)loadContentScrollView {
    ...
    [tableView registerClass:[IMLEXDBQueryRowCell class]
        forCellReuseIdentifier:kIMLEXDBQueryRowCellReuse
    ];
...
- (void)loadLeftView {
    ...
    leftHeader.backgroundColor = IMLEXColor.secondaryBackgroundColor;
...
- (void)loadHeaderData {
    ...
    for (NSInteger column = 0; column < self.numberOfColumns; column++) {
        ...
        IMLEXTableColumnHeader *header = [[IMLEXTableColumnHeader alloc]
...
- (void)contentHeaderTap:(UIGestureRecognizer *)gesture {
    ...
    IMLEXTableColumnHeaderSortType newType = IMLEXNextTableColumnHeaderSortType(self.sortType);

    // Reset old header
    IMLEXTableColumnHeader *oldHeader = (id)self.headerScrollView.subviews[self.sortColumn];
    oldHeader.sortType = IMLEXTableColumnHeaderSortTypeNone;

    // Update new header
    IMLEXTableColumnHeader *newHeader = (id)self.headerScrollView.subviews[newSortColumn];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    UIColor *backgroundColor = IMLEXColor.primaryBackgroundColor;
    if (indexPath.row % 2 != 0) {
        backgroundColor = IMLEXColor.secondaryBackgroundColor;
    }

    // Left side table view for row numbers
    if (tableView == self.leftTableView) {
        IMLEXTableLeftCell *cell = [IMLEXTableLeftCell cellWithTableView:tableView];
    ...
    // Right side table view for data
        ...
        IMLEXDBQueryRowCell *cell = [tableView
            dequeueReusableCellWithIdentifier:kIMLEXDBQueryRowCellReuse forIndexPath:indexPath
        ];
