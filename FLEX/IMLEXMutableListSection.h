...
typedef void (^IMLEXMutableListCellForElement)(__kindof UITableViewCell *cell, id element, NSInteger row);
...
///
/// By default, \c kIMLEXDetailCell is the reuse identifier used. If you need
...
@interface IMLEXMutableListSection<__covariant ObjectType> : IMLEXCollectionContentSection

/// Initializes a section with an empty list.
+ (instancetype)list:(NSArray<ObjectType> *)list
   cellConfiguration:(IMLEXMutableListCellForElement)configurationBlock
...
/// A readwrite version of the same property in \c IMLEXTableViewSection.h
///
