...
@implementation IMLEXUtility

+ (UIWindow *)appKeyWindow {
    // First, check UIApplication.keyWindow
    IMLEXWindow *window = (id)UIApplication.sharedApplication.keyWindow;
    if (window) {
    ...
    //
    // Also, these are obviously not all IMLEXWindows; IMLEXWindow is used
    // so we can call window.previousKeyWindow without an ugly cast
    for (IMLEXWindow *window in UIApplication.sharedApplication.windows) {
...
+ (UIColor *)hierarchyIndentPatternColor {
    ...
    dispatch_once(&onceToken, ^{
        UIImage *indentationPatternImage = IMLEXResources.hierarchyIndentPattern;
...
+ (NSString *)applicationName {
    return IMLEXUtility.applicationImageName.lastPathComponent;
}
...
+ (SEL)swizzledSelectorForSelector:(SEL)selector {
    return NSSelectorFromString([NSString stringWithFormat:
        @"_IMLEX_swizzle_%x_%@", arc4random(), NSStringFromSelector(selector)
    ]);
}
