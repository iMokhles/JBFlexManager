//
//  IMLEXLiveObjectsTableViewController.m
//  Flipboard
//
...
static const NSInteger kIMLEXLiveObjectsSortAlphabeticallyIndex = 0;
static const NSInteger kIMLEXLiveObjectsSortByCountIndex = 1;
static const NSInteger kIMLEXLiveObjectsSortBySizeIndex = 2;

@interface IMLEXLiveObjectsTableViewController ()
...
@implementation IMLEXLiveObjectsTableViewController
...
+ (UIViewController *)globalsEntryViewController:(IMLEXGlobalsRow)row {
    IMLEXLiveObjectsTableViewController *liveObjectsViewController = [self new];
...
- (void)updateSearchResults:(NSString *)filter {
    ...
    if (selectedScope == kIMLEXLiveObjectsSortAlphabeticallyIndex) {
        self.filteredClassNames = [self.filteredClassNames sortedArrayUsingSelector:@selector(caseInsensitiveCompare:)];
    } else if (selectedScope == kIMLEXLiveObjectsSortByCountIndex) {
    ...
    } else if (selectedScope == kIMLEXLiveObjectsSortBySizeIndex) {
        self.filteredClassNames = [self.filteredClassNames sortedArrayUsingComparator:^NSComparisonResult(NSString *className1, NSString *className2) {
            NSNumber *count1 = self.instanceCountsForClassNames[className1];
            NSNumber *count2 = self.instanceCountsForClassNames[className2];
...
- (UITableViewCell *)tableView:(__kindof UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    UITableViewCell *cell = [tableView
        dequeueReusableCellWithIdentifier:kIMLEXDefaultCell
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    NSString *className = self.filteredClassNames[indexPath.row];
    IMLEXObjectListViewController *instancesViewController = [IMLEXObjectListViewController instancesOfClassWithName:className];
