...
@interface IMLEXExplorerToolbarItem ()

@property (nonatomic) IMLEXExplorerToolbarItem *sibling;
...
@implementation IMLEXExplorerToolbarItem
...
+ (instancetype)itemWithTitle:(NSString *)title image:(UIImage *)image sibling:(IMLEXExplorerToolbarItem *)backupItem {
    NSParameterAssert(title); NSParameterAssert(image);
    
    IMLEXExplorerToolbarItem *toolbarItem = [self buttonWithType:UIButtonTypeSystem];
    toolbarItem.sibling = backupItem;
    toolbarItem.title = title;
    toolbarItem.image = image;
    toolbarItem.tintColor = IMLEXColor.iconColor;
    ...
    [toolbarItem setTitleColor:IMLEXColor.primaryTextColor forState:UIControlStateNormal];
    [toolbarItem setTitleColor:IMLEXColor.deemphasizedTextColor forState:UIControlStateDisabled];
    return toolbarItem;
}

- (IMLEXExplorerToolbarItem *)currentItem {
...
+ (UIColor *)highlightedBackgroundColor {
    return IMLEXColor.toolbarItemHighlightedColor;
}

+ (UIColor *)selectedBackgroundColor {
    return IMLEXColor.toolbarItemSelectedColor;
}
...
- (CGRect)titleRectForContentRect:(CGRect)contentRect {
    ...
    titleRect.origin.x = contentRect.origin.x + IMLEXFloor((contentRect.size.width - titleSize.width) / 2.0);
    return titleRect;
}
...
- (CGRect)imageRectForContentRect:(CGRect)contentRect {
    ...
    CGFloat originY = [[self class] topMargin] + IMLEXFloor((availableHeight - imageSize.height) / 2.0);
    CGFloat originX = IMLEXFloor((contentRect.size.width - imageSize.width) / 2.0);
