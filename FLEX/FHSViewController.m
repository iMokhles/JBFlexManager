...
BOOL const kFHSViewControllerExcludeIMLEXWindows = YES;
...
- (id)initWithViews:(NSArray<UIView *> *)views
    ...
    if (self) {
        ...
        if (!viewsAtTap && kFHSViewControllerExcludeIMLEXWindows) {
            Class IMLEXwindow = [IMLEXWindow class];
            views = [views IMLEX_filtered:^BOOL(UIView *view, NSUInteger idx) {
                return [view class] != IMLEXwindow;
            }];
        }

        _targetViews = views;
        _views = [views IMLEX_mapped:^id(UIView *view, NSUInteger idx) {
...
- (void)refreshSnapshotView {
    ...
    UIAlertController *loading = [IMLEXAlert makeAlert:^(IMLEXAlert *make) {
        make.title(@"Please Wait").message(@"Generating snapshot…");
    }];
    [self presentViewController:loading animated:YES completion:^{
        self.snapshots = [self.views IMLEX_mapped:^id(FHSView *view, NSUInteger idx) {
...
- (void)loadView {
    [super loadView];
    self.view.backgroundColor = IMLEXColor.primaryBackgroundColor;
}
...
- (void)viewDidLoad {
    ...
    self.navigationItem.leftBarButtonItem = [UIBarButtonItem
        itemWithImage:IMLEXResources.toggle2DIcon
...
- (void)setSnapshotView:(FHSSnapshotView *)snapshotView {
    ...
    self.toolbarItems = @[
        ...
        [UIBarButtonItem
            itemWithImage:IMLEXResources.moreIcon
