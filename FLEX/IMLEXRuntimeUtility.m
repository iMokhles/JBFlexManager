...
#import "IMLEXObjcInternal.h"
#import "IMLEXTypeEncodingParser.h"

static NSString *const IMLEXRuntimeUtilityErrorDomain = @"IMLEXRuntimeUtilityErrorDomain";
typedef NS_ENUM(NSInteger, IMLEXRuntimeUtilityErrorCode) {
    IMLEXRuntimeUtilityErrorCodeDoesNotRecognizeSelector = 0,
    IMLEXRuntimeUtilityErrorCodeInvocationFailed = 1,
    IMLEXRuntimeUtilityErrorCodeArgumentTypeMismatch = 2
};

@implementation IMLEXRuntimeUtility

#pragma mark - General Helpers (Public)

+ (BOOL)pointerIsValidObjcObject:(const void *)pointer {
    return IMLEXPointerIsValidObjcObject(pointer);
}
...
+ (id)potentiallyUnwrapBoxedPointer:(id)returnedObjectOrNil type:(const IMLEXTypeEncoding *)returnType {
    ...
    NSInteger i = 0;
    if (returnType[i] == IMLEXTypeEncodingConst) {
        i++;
    }

    BOOL returnsObjectOrClass = returnType[i] == IMLEXTypeEncodingObjcObject ||
                                returnType[i] == IMLEXTypeEncodingObjcClass;
    BOOL returnsVoidPointer   = returnType[i] == IMLEXTypeEncodingPointer &&
                                returnType[i+1] == IMLEXTypeEncodingVoid;
    BOOL returnsCString       = returnType[i] == IMLEXTypeEncodingCString;
    ...
    if (!returnsObjectOrClass) {
        ...
        if (returnsCString) {
            ...
            if ([IMLEXRuntimeUtility pointerIsValidObjcObject:value.pointerValue]) {
...
+ (NSUInteger)fieldNameOffsetForTypeEncoding:(const IMLEXTypeEncoding *)typeEncoding {
    ...
    while (typeEncoding[beginIndex] == IMLEXTypeEncodingQuote) {
        NSUInteger endIndex = beginIndex + 1;
        while (typeEncoding[endIndex] != IMLEXTypeEncodingQuote) {
            ++endIndex;
        }
...
+ (NSArray<NSString *> *)allPropertyAttributeKeys {
    ...
    dispatch_once(&onceToken, ^{
        allPropertyAttributeKeys = @[
            kIMLEXPropertyAttributeKeyTypeEncoding,
            kIMLEXPropertyAttributeKeyBackingIvarName,
            kIMLEXPropertyAttributeKeyReadOnly,
            kIMLEXPropertyAttributeKeyCopy,
            kIMLEXPropertyAttributeKeyRetain,
            kIMLEXPropertyAttributeKeyNonAtomic,
            kIMLEXPropertyAttributeKeyCustomGetter,
            kIMLEXPropertyAttributeKeyCustomSetter,
            kIMLEXPropertyAttributeKeyDynamic,
            kIMLEXPropertyAttributeKeyWeak,
            kIMLEXPropertyAttributeKeyGarbageCollectable,
...
+ (NSArray<NSString *> *)prettyArgumentComponentsForMethod:(Method)method {
    ...
    for (unsigned int argIndex = 0; argIndex < selectorComponents.count; argIndex++) {
        char *argType = method_copyArgumentType(method, argIndex + kIMLEXNumberOfImplicitArgs);
...
+ (id)performSelector:(SEL)selector
    ...
    if (![object respondsToSelector:selector]) {
        if (error) {
            ...
            *error = [NSError
                errorWithDomain:IMLEXRuntimeUtilityErrorDomain
                code:IMLEXRuntimeUtilityErrorCodeDoesNotRecognizeSelector
                userInfo:userInfo
            ];
        }
    ...
    if (!methodSignature.methodReturnLength &&
        methodSignature.methodReturnType[0] != IMLEXTypeEncodingVoid) {
        return nil;
    }
    ...
    for (NSUInteger argumentIndex = kIMLEXNumberOfImplicitArgs; argumentIndex < numberOfArguments; argumentIndex++) {
        NSUInteger argumentsArrayIndex = argumentIndex - kIMLEXNumberOfImplicitArgs;
        ...
        if (argumentObject && ![argumentObject isKindOfClass:[NSNull class]]) {
            ...
            if (typeEncodingCString[0] == IMLEXTypeEncodingObjcObject ||
              typeEncodingCString[0] == IMLEXTypeEncodingObjcClass ||
              [self isTollFreeBridgedValue:argumentObject forCFType:typeEncodingCString]) {
                ...
                if (strcmp([argumentValue objCType], typeEncodingCString) != 0) {
                    if (error) {
                        ...
                        *error = [NSError
                            errorWithDomain:IMLEXRuntimeUtilityErrorDomain
                            code:IMLEXRuntimeUtilityErrorCodeArgumentTypeMismatch
                            userInfo:userInfo
                        ];
                    }
                    return nil;
                ...
                @try {
                    ...
                    IMLEXGetSizeAndAlignment(typeEncodingCString, &bufferSize, NULL);
    ...
    @try {
        ...
        if (returnType[0] == IMLEXTypeEncodingObjcObject || returnType[0] == IMLEXTypeEncodingObjcClass) {
        ...
        } else if (returnType[0] != IMLEXTypeEncodingVoid) {
            NSAssert(methodSignature.methodReturnLength, @"Memory corruption lies ahead");

            if (returnType[0] == IMLEXTypeEncodingStructBegin) {
                if (selector == stdStringExclusion && [object isKindOfClass:[NSString class]]) {
                    ...
                    if (error) {
                        *error = [NSError
                            errorWithDomain:IMLEXRuntimeUtilityErrorDomain
                            code:IMLEXRuntimeUtilityErrorCodeInvocationFailed
...
+ (NSValue *)valueForNumberWithObjCType:(const char *)typeEncoding fromInputString:(NSString *)inputString {
    ...
    if (strlen(typeEncoding) > 1) {
        ...
        if ([type isEqualToString:@IMLEXEncodeClass(NSDecimalNumber)]) {
            return [NSDecimalNumber decimalNumberWithString:inputString];
        } else if ([type isEqualToString:@IMLEXEncodeClass(NSNumber)]) {
            return number;
        }
...
+ (void)enumerateTypesInStructEncoding:(const char *)structEncoding
    ...
    if (structEncoding && structEncoding[0] == IMLEXTypeEncodingStructBegin) {
        ...
        if (equals) {
            ...
            if (IMLEXGetSizeAndAlignment(structEncoding, &structSize, &fieldAlignment)) {
                NSUInteger runningFieldIndex = 0;
                NSUInteger runningFieldOffset = 0;
                const char *typeStart = equals + 1;
                
                while (*typeStart != IMLEXTypeEncodingStructEnd) {
                    NSUInteger fieldSize = 0;
                    // If the struct type encoding was successfully handled by
                    // IMLEXGetSizeAndAlignment above, we *should* be ok with the field here.
...
+ (NSString *)appendName:(NSString *)name toType:(NSString *)type {
    ...
    if ([type characterAtIndex:type.length - 1] == IMLEXTypeEncodingCString) {
...
+ (NSString *)readableTypeForEncoding:(NSString *)encodingString {
    ...
    const NSUInteger fieldNameOffset = [IMLEXRuntimeUtility fieldNameOffsetForTypeEncoding:encodingCString];
    if (fieldNameOffset > 0) {
        ...
        NSArray<NSString *> *const fieldNames = [fieldNamesString
            componentsSeparatedByString:[NSString stringWithFormat:@"%c", IMLEXTypeEncodingQuote]
        ];
    ...
    // Objects
    if (encodingCString[0] == IMLEXTypeEncodingObjcObject) {
        NSString *class = [encodingString substringFromIndex:1];
        class = [class stringByReplacingOccurrencesOfString:@"\"" withString:@""];
        if (class.length == 0 || (class.length == 1 && [class characterAtIndex:0] == IMLEXTypeEncodingUnknown)) {
            class = @"id";
        } else {
...
#undef TRANSLATE
    ...
    if (encodingCString[0] == IMLEXTypeEncodingStructBegin) {
        ...
        if (equals) {
            ...
            if (nameStart[0] == IMLEXTypeEncodingUnknown) {
...
+ (NSValue *)valueForPrimitivePointer:(void *)pointer objCType:(const char *)type {
    ...
    const NSUInteger fieldNameOffset = [IMLEXRuntimeUtility fieldNameOffsetForTypeEncoding:type];
    if (fieldNameOffset > 0) {
...
#undef CASE
    ...
    NSValue *value = nil;
    if (IMLEXGetSizeAndAlignment(type, nil, nil)) {
        @try {
