...
@interface JBFlexManagerListController: PSListController {
}
@end

@implementation JBFlexManagerListController
- (id)specifiers {
	if(_specifiers == nil) {
		_specifiers = [[self loadSpecifiersFromPlistName:@"JBFlexManager" target:self] retain];
	}
	return _specifiers;
}

- (void)followOnTwitter:(PSSpecifier*)specifier {
    // PFStatusBarAlert *alertStatus = [[PFStatusBarAlert alloc] initWithMessage:@"Testing" notification:nil action:@selector(shareTapped:) target:self];
    // [alertStatus showOverlayForSeconds:10];
...
{
	[super loadView];
	UIBarButtonItem *tweet = [[UIBarButtonItem alloc] initWithImage:[UIImage imageWithContentsOfFile:@"/Library/PreferenceBundles/JBFlexManager.bundle/Twitter.png"] style:UIBarButtonItemStylePlain target:self action:@selector(shareTapped:)];
    ...
    creditLabel.text = @"FlexManager Debugging Tools for JB";
...
{
	NSString *text = @"get #FlexDebugging enabled in your jailbroken devices with #JBFlexManager to debug more stuffs programmed by @iMokhles";
	// NSURL *url = [NSURL URLWithString:@"http://moreinfo.thebigboss.org/moreinfo/depiction.php?file=revealmenuDp"];
